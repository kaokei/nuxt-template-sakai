import type { Menu, MenuTreeNode } from '@sakai/services/MenuService';
...
export class MenuMgrService {
  ...
  async loadMenus(keyword?: string): Promise<void> {
    ...
    try {
      ...
      this.allMenus = await this.menuService.queryMenus(k);
  ...
  async onDeleteConfirm(): Promise<DeleteResult> {
    if (this.deleteTarget) {
      const result = await this.menuService.deleteMenu(this.deleteTarget.key);
    ...
    const deletedCount = await this.menuService.deleteMenus(ids);
    this.selectedNodes = {};
    this.deleteDialogVisible = false;
    this.loadMenus();
