...
import { generateLoginLogs } from '../data/login-logs';
import type { LoginLog } from '../data/login-logs';

let logs = generateLoginLogs(80);

export const loginLogHandlers = [
  // 查询列表
  http.get('/api/login-logs', async ({ request }) => {
    ...
    // 时间范围筛选
    const dateFrom = searchParams.loginTimeFrom;
    const dateTo = searchParams.loginTimeTo;
    delete searchParams.loginTimeFrom;
    delete searchParams.loginTimeTo;
    if (dateFrom || dateTo) {
      const fromMs = dateFrom ? Date.parse(dateFrom) : NaN;
      const toMs = dateTo ? Date.parse(dateTo) : NaN;
      filtered = filtered.filter((l) => {
        const c = new Date(l.loginTime).getTime();
  ...
  // 选项数据（状态选项供搜索下拉使用）
  // ⚠️ 必须在 /api/login-logs/:id 之前，避免 options 被当作 id 匹配
  http.get('/api/login-logs/options', async () => {
    await delay(100);
  ...
  // 查询单条详情
  http.get('/api/login-logs/:id', async ({ params }) => {
    const { id } = params;
